...
with spine as (
    select object_ref accrual_schedule_id, period_id, legal_entity_id,
    ...
    where subledger_domain = 'ACCRUAL'
    group by object_ref, period_id, legal_entity_id
)
select
    md5(concat_ws('|', schedule.period_id, schedule.accrual_schedule_id)) as accrual_schedule_hk,
...
from {{ ref('stg_working_capital__accrual_schedule') }} schedule
...
left join spine
  on schedule.accrual_schedule_id = spine.accrual_schedule_id

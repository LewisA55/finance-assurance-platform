...
with spine as (
    select object_ref debt_instrument_id, period_id, legal_entity_id,
    ...
    where subledger_domain = 'DEBT'
    group by object_ref, period_id, legal_entity_id
)
select
    md5(concat_ws('|', schedule.period_id, schedule.debt_instrument_id)) as debt_schedule_hk,
...
from {{ ref('stg_treasury__debt_schedule') }} schedule
inner join {{ ref('dim_debt_instrument') }} debt using (debt_instrument_id, legal_entity_id)
...
left join spine
  on schedule.debt_instrument_id = spine.debt_instrument_id
